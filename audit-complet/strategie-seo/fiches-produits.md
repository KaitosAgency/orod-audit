...
1. **Prévenir le "Cumulative Layout Shift" (CLS) :**
   * _Le contexte :_ Avec **`srcset`** / **`<picture>`** responsive, il est fréquent que la balise `<img>` n'affiche pas toutes les dimensions finales en dur ; le navigateur choisit la source selon la viewport. Si l'espace n'est pas réservé, le contenu « saute » au chargement, ce qui pénalise les Core Web Vitals.
   * _La solution recommandée :_ Combiner **`width` et `height` intrinsèques** sur `<img>` lorsque c'est cohérent avec le pipeline d'images (pour que le navigateur déduise le ratio avant le chargement), **et/ou** imposer un **ratio fixe via CSS** sur le conteneur (`aspect-ratio: 1 / 1` pour les vignettes carrées, etc.) ou utiliser un **wrapper** qui réserve la hauteur. L'objectif est que la mise en page soit stable **avant** le décodage complet de l'image.
2. **Affiner la sémantique de l'attribut `alt` (marge de progression SEO) :**
   * _Le constat :_ L'attribut `alt` actuel est la copie stricte du titre produit, avec parfois la référence en premier (ex: `alt="580460 - medaille courage et devouement..."`). Ce n'est **pas une erreur bloquante** : le texte reste descriptif et les balises `alt` sont présentes.
   * _La piste d'amélioration :_ Pour Google Images et l'accessibilité, on peut gagner en pertinence en plaçant la **description visuelle** en tête et en évitant de commencer systématiquement par le numéro de référence (ex: `alt="Médaille courage et dévouement BR Vermeil en boite plastique"`).
