...
**Où le brancher techniquement (home modulaire) :** La page d’accueil étant en principe **assemblée par blocs** (grilles produits, mises en avant, carrousels, etc.), il n’y a pas un seul « endroit magique » fixe dans le HTML statique. Le JSON-LD **`ItemList` doit donc être produit au plus près du rendu** : idéalement **dans le même module / composant que la grille produits** (ou le bloc CMS équivalent) qui charge la sélection affichée. Ainsi, si vous changez l’ordre des blocs, dupliquez une grille ou remplacez une sélection depuis le back-office, le marquage structuré reste **aligné** sur ce qui est réellement montré à l’utilisateur — sans divergence entre grilles HTML et données structurées. Sur la home, plusieurs sections produits peuvent donner **plusieurs listes logiques** (une par bloc pertinente) ou une agrégation en un seul script, selon le choix d’architecture ; l’important est que chaque liste reflète fidèlement les produits du bloc concerné.
