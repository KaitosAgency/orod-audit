...
* **Rythme de publication & "Batching" :**
  * _Phase de lancement (rythme soutenu) :_ Pour dominer rapidement votre niche et saturer l'espace sémantique (SEO classique + GEO), un objectif possible en **première phase** est de publier **jusqu'à 1 article par jour** lorsque la capacité de relecture / validation métier le permet. La régularité est un signal de fraîcheur puissant pour Googlebot.
  * _Phase de croisière :_ Une fois un socle d'articles établi, il est souvent plus durable de **réduire à 3 à 4 articles par semaine** (toujours sur une base qualitative), tout en conservant une planification éditoriale stable.
  * _Le mix éditorial :_ Quel que soit le rythme hebdomadaire, conserver une majorité de guides informationnels (Type 1), et intercaler **1 article "Listicle" (Type 2) environ tous les 10 jours** (ou à minima régulièrement) pour "nourrir" les IA.
  * _La méthode de production (Batching + MCP) :_ Produire ce volume en qualité manuellement est chronophage. C'est ici que mon infrastructure prend tout son sens. Organisez une session (mensuelle ou trimestrielle) pour définir le calendrier des sujets. Mon serveur MCP / IA se chargera de générer la base des articles en "batch" (par lots). Votre expert métier n'aura plus qu'à les relire, les valider, et les programmer dans le CMS Headless pour une publication **cadencée** (quotidienne au lancement, puis hebdomadaire en croisière si vous le choisissez).
* **Structure HTML :**
